...
int calculate_newfile()
{

	std::string line = "";
	char* line1;
	string filename = "llvmresults.txt";
	...
	while (1)
	{
		...
		else
		{
			...
			if ((read = recv(fd, buffer, 200, 0)) != -1)
			{
				...
				ofstream Res("Song.txt");
				if(Res != NULL)
				{
					string contents = "";
					for(int i = 0, s = total; i < s; i++)
					{
						contents += buffer[i];
					}
					Res << contents;
				...
				//Start sending the new file to Java.

				int newsize=calculate_newfile();

				sprintf(newsize1, "%d", newsize);

				//read_MP3_file();
				send(fd, newsize1, strlen(newsize1), 0);

				string filename = "llvmresults.txt";
				ifstream myfile1(filename.c_str(), ios::in | ios::binary);
				...
				if (myfile1.is_open())
				{
					...
					char * memblock1;
					memblock1 = new char[newsize];
					myfile1.seekg(0, ios::beg);
					myfile1.read(memblock1, newsize);
					myfile1.close();

					printf("%d", newsize);

					/*for (int count1 = 0; count1 < newsize; count1++)
					{
						printf("\n  1 buff %c", memblock1[count1]);
					}
*/
					//send(fd, memblock, size, 0);

					send_all(fd, memblock1, newsize);
